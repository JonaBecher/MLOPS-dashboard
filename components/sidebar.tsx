...
export default function Sidebar(props: Props) {
    ...
    const Menus = [
        {
            title: "Dashboard", icon: DashboardIcon, gap: false, onclick: () => {
                router.push('/' + projectId + '/dashboard')
            }
        },
        {
            title: "Models", icon: CubeIcon, gap: false, onclick: () => {
                router.push('/' + projectId + '/models')
            }
        },
        {
            title: "Devices", icon: LaptopIcon, gap: false, onclick: () => {
                router.replace('/' + projectId + '/devices')
            }
        }
    ];
    ...
    return (
        <div className="flex">
            <div className={`${open ? "w-72" : "w-28"} p-5 pt-8 h-screen bg-gray-700 relative duration-300`}>
                    ...
                    <div className={`${!open && 'pl-2'} duration-200 flex items-center gap-x-2 max-h-10 cursor-pointer`}
                         onClick={() => router.push('/')}>
                        <div
                            style={{minWidth: "52px", minHeight: "52px"}}
                            className="flex rounded-lg justify-center items-center bg-blue-700 text-white text-xl">
                            E
                        </div>
                    ...
                    <ul className={`pt-10 ${!open && 'pl-2'} duration-200`}>
                        {Menus.map((Menu, index) => (
                            ...
                            >
                                <Menu.icon className="font-bold min-w-7 min-h-7" color="white" stroke={"4"} width={24}
                                           height={24}/>
                                <span
                                    className={`${!open && "hidden"} origin-left duration-200 font-medium text-lg text-white`}>
                                    {Menu.title}
                                  </span>
                            </li>
                        ))}
